...
// destroy
template <class _T> void _Destroy_one(_T *, std::true_type) {}

template <class _T> void _Destroy_one(_T *__p, std::false_type) {
  if (__p != nullptr) {
    __p->~_T();
  }
}
...
template <class _T> void destory(_T *__first) {
