...
{
    whlie (__first != __last)
        *(--__result) = *(--__lasst);
    return __result;
}

template <class _RandomIter, class _BidirectionalIter>
_BidirectionalIter __copy_backward(_RandomIter __first,
                                    _RandomIter __last,
...
typename std::enable_if<
    std::is_integral<_ForwardIter>::value && sizeof(_ForwardIter) == 1 &&
...
template <class _ForwardIter, class _Tp>
void fill(_FrowardIterator __first, _ForwardIterator __last, const _Tp& __value)
{
